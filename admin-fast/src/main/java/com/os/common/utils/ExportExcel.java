...
public class ExportExcel {
    ...
    //生成一个sheet
	public static void download(HttpServletRequest request, HttpServletResponse response, List<Map<String, Object>> list, List<String> heads, String name, String condition, boolean isSeq) throws Exception {
			ExcelHeadXML xml=getExcelHeadXML(name, heads, isSeq, true);
			//实际调用
			toExport(list, xml,condition, request, response,"");
	}

	//生成一个sheet
	public static void download(ZipOutputStream zip, List<Map<String, Object>> list, List<String> heads, String name, String condition) throws Exception {
			ExcelHeadXML xml=getExcelHeadXML(name, heads, true, true);
			//实际调用
			toExport(zip, list, xml,condition,"");
	}
...
*/
	...
	public static void toExport(List<Map<String, Object>> list, ExcelHeadXML xml, String condition, HttpServletRequest request, HttpServletResponse response, String picPath)
		...
		ExportExcel excel = new ExportExcel(false);
		XSSFWorkbook workBook = excel.createWorkBook(list, xml,sheet1,condition,picPath);
		doExport(workBook, xml.getTitle(), request, response);
	}

	public static void toExport(ZipOutputStream zip, List<Map<String, Object>> list, ExcelHeadXML xml, String condition, String picPath)
			throws IOException, SQLException {
		ExportExcel excel = new ExportExcel(false);
		XSSFWorkbook workBook = excel.createWorkBook(list, xml,sheet1,condition,picPath);
		workBook.write(zip);
	...
	private void createTop(final ExcelHeadXML xml, XSSFSheet sheet, String condition) {
		...
		if (null != head && head.size() > 0) {
			...
			for (int i = 0; i < head.size(); i++) {
				sheet.setColumnWidth(i, 13 * 256); //第一个参数代表列id(从0开始),第2个参数代表宽度值
